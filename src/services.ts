...
import { AvnuOptions, Compatible, ExecuteCallsOptions, GasTokenPrice, InvokeResponse, RequestError } from './types';

const baseUrl = (options?: AvnuOptions): string =>
  options?.baseUrl ?? (process.env.NODE_ENV === 'dev' ? TESTNET_BASE_URL : BASE_URL);
const getRequest = (options?: AvnuOptions): RequestInit => ({
  signal: options?.abortSignal,
  headers: { ...(options?.avnuPublicKey !== undefined && { 'ask-signature': 'true' }) },
});
const postRequest = (body: unknown, options?: AvnuOptions): RequestInit => ({
  method: 'POST',
  headers: {
 ...
 */
const fetchIsCompatible = (accountAddress: string, options?: AvnuOptions): Promise<boolean> =>
  fetch(`${baseUrl(options)}/gasless/v1/accounts/${accountAddress}/compatible`, getRequest(options))
    .then((response) => parseResponse<Compatible>(response, options?.avnuPublicKey))
 ...
 * @param options Optional options
 */
const fetchGasTokenPrices = (options?: AvnuOptions): Promise<GasTokenPrice[]> =>
  fetch(`${baseUrl(options)}/gasless/v1/gas-token-prices`, getRequest(options))
    .then((response) => parseResponse<GasTokenPrice[]>(response, options?.avnuPublicKey))
    .then((gasTokenPrices) =>
...
const fetchExecuteTransaction = (
  ...
  ).then((response) => parseResponse<InvokeResponse>(response, options?.avnuPublicKey));
};
